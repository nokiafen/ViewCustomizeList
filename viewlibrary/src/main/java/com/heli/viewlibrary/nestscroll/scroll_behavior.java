package com.heli.viewlibrary.nestscroll;
...
//        parent.findViewById(R.id.title_bar).setAlpha(1-perchent);
    ...
    public boolean onLayoutChild(@NonNull CoordinatorLayout parent, @NonNull NestedScrollView child,
        ...
        animRoot = parent.findViewById(R.id.anim_root);
    ...
    public void onNestedPreScroll(@NonNull CoordinatorLayout coordinatorLayout,
        ...
        if (dy > 0 && dependView.getTranslationY() <= 0 && anim_root.getTranslationY() == 0) {   //刷新布局未偏移 ， 向上滑动 且 上部分折叠区未折叠或正在折叠
...
//            if (isNestScrolling) { //防止 向下滑动 同时触发刷新
...
//            }
           ...
           calculate+=(calculate/anim_root.getMeasuredHeight())*dy; //阻尼效果
            ...
            Log.d("onNestedPreScroll_down", dy + "");
    ...
    private void onRefreshEnd() {
        if (anim_root.getTranslationY() <= 0) { //控件拖出来了才处理
            return;
        }
        scroller.abortAnimation(); //切断其它动画
        if (anim_root.getTranslationY() >= anim_root.getMeasuredHeight() * 0.5f) { //拖出一般则全部拖出
            //todo
            scroller.startScroll(0, (int) anim_root.getTranslationY(), 0, anim_root.getMeasuredHeight() - (int) anim_root.getTranslationY(), 250);
        } else { //拖出部分则收回去
            scroller.startScroll(0, (int) anim_root.getTranslationY(), 0, 0 - (int) anim_root.getTranslationY(), 250);
        }
    ...
    private Runnable refreshEndRunnalbe = new Runnable() {
        ...
        @Override
        public void run() {
            if (scroller.computeScrollOffset()) {  //滑动动画执行中
                anim_root.setTranslationY(scroller.getCurrY());
                scroll_content.setTranslationY(scroller.getCurrY());
                handler.post(this);
            } else { //滑动结束
                if (anim_root.getTranslationY() > 0) {//滑动结束且偏移量未正 则执行刷新动画
