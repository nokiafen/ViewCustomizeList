package com.example.scrollconflictdemo.Views;
...
import com.example.scrollconflictdemo.utils.Util;
...
public class VerticalBar extends View {
  ...
  public boolean onTouchEvent(MotionEvent event) {
    ...
    progress = progress >= 100 ? 100 : progress; //计算当前点击处 此时对应的progress

    switch (event.getAction()) {
      case MotionEvent.ACTION_UP:  //up事件 釋放
        setProgress((int) progress);
        getParent().requestDisallowInterceptTouchEvent(false);
        veticalDrad = false;
      case MotionEvent.ACTION_CANCEL://cancle 事件 释放
        veticalDrad = false;
        break;
      case MotionEvent.ACTION_DOWN: //事件一开始 先请求父组件不拦截接下来的事件

        downY = event.getY();
        getParent().requestDisallowInterceptTouchEvent(true);
        return true;
      case MotionEvent.ACTION_MOVE:  //！important
        if (veticalDrad) {  //如果已经判断是上下滑动事件
          setProgress((int) progress);
          getParent().requestDisallowInterceptTouchEvent(true);
        } else {  //当前状态 判断不是正在上下滑动事件
          if (Math.abs(event.getY() - downY)
              >= ViewConfiguration.get(getContext()).getScaledTouchSlop() * 2) { //判斷與down時的y方向上的位移是否大於最小偏移量
            setProgress((int) progress);
            getParent().requestDisallowInterceptTouchEvent(true);
            veticalDrad = true;  //標記當前滑動是上下滑動事件
          } else {//標記當前滑動非 上下滑動事件，把事件還給父控件
            getParent().requestDisallowInterceptTouchEvent(false);
            veticalDrad = false;
          }
        }
